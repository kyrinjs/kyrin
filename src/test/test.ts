import type { Handler, HttpMethod } from "../core/types";
...
export class TestApp {
  ...
  private toResponse(result: Handler | Response | unknown, ctx: Context): Response {
...
export function expect(value: unknown): AssertionResult {
  return {
    toBe(expected: number) {
      if (value !== expected) {
    ...
    toContain(expected: string) {
      if (typeof value !== "string" || !value.includes(expected)) {
        throw new Error(`Expected "${expected}" to be contained in "${value}"`);
      }
    },
    toBeTruthy() {
      if (!value) {
    ...
    toThrow(error?: Error | string | RegExp) {
      if (typeof value !== "function") {
        throw new Error("Expected must be a function to test throw");
      }
      try {
        (value as () => void)();
      } catch (e) {
        if (error instanceof Error && (e as Error).message !== error.message) {
          throw new Error(`Expected error message "${error.message}", got "${(e as Error).message}"`);
        }
        if (typeof error === "string" && (e as Error).message !== error) {
          throw new Error(`Expected error message "${error}", got "${(e as Error).message}"`);
        }
        if (error instanceof RegExp && !error.test((e as Error).message)) {
          throw new Error(`Expected error message to match ${error}, got "${(e as Error).message}"`);
        }
        return;
      }
...
export const mocking = {
  ...
  createMockContext: _createMockContext,
};
